...
public class LoginUserUseCaseImpl implements LoginUserUseCase {
	...
	public UserDTO execute(UserDTO user) {
		...
		boolean isEquals = !findUserUseCase.findUser(user).isEmpty();
		...
		if (!isEquals) {
			throw ServiceCustomException
					.createUserException(Messages.LoginUserUseCaseImpl.BUSSINES_USER_INCORRECT_PASSWORD);
		}
	...
	public static void main(String[] args) {
		...
		users.forEach(elem -> System.out.println("the user is id:" + elem.getIdAsString() + "the mail is: "
				+ "the name user is: [" + elem.getName() + "] whith email is: " + elem.getEmail()
				+ " the document identidad: " + elem.getIdentification()));
	}
